...
function compoundInterest(principle,rate,years) {
  let ci = principle * Math.exp(rate*years);
  return ci;
}


/* Define a function fizzBuzz() that takes in a single number as an argument.
   The function should return an array of numbers from 1 to the argument.
...
/* Define a function getLetterFrequencies() that takes in a single string as
   ...
   Hint: start with an empty Object, then loop through the letters one at a
...
/* Create a variable deck that represents a deck of modern playing cards
   ...
   array of 52 elements, each of which is an Object with properties:
    ...
    var deck = [];
    var suitOptions = ["hearts","diamonds","clubs","spades"];
    for(var i = 0; i < 4; i++) {
      for(var j = 2; j<=14; j++) {
        var obj = {};
        obj.suit = suitOptions[i];
        obj.rank = j;
        deck.push(obj);
      }
    }
...
/* Define a function getHighCard() that takes in an array of "card" objects
  ...
  function getHighCard(array) {
    ...
    for(var i = 1; i<array.length; i++){
      if(array[i].rank > array[index].rank) {
        index = i;
      }
    }
    return array[index];
...
/* Define a function isFlush() that takes in an array of "card" objects and
   returns whether or not the cards all have the same suit. */
...
/* Extra challenge: define a function hasPair() that takes in an array of
   "card" objects and returns whether or not there is at least one pair (two
   cards with the same rank) in the array.
