...
// console.log(extractOpponent({"date":"9/3/16", "home":"UW", "opponent":"Rutgers", "home_score":48,
// "opponent_score":13, "passing_yards":289, "rushing_yards":91, "fumbles":0 },))
...
let array = huskyGames2016.map(extractOpponent);
console.log(array);

//Define a function huskiesLost() that takes in a "game" object and returns
//whether or not UW lost.
function huskiesLost(obj) {
  if (obj.home == "UW" && obj.home_score < obj.opponent_score) {
    return obj.opponent;
  } else if (obj.opponent == "UW" && obj.opponent_score < obj.home_score) {
    return obj.home;
  }
}

// console.log(huskiesLost({"date":"9/3/16", "home":"UW", "opponent":"Rutgers", "home_score":48,
//  "opponent_score":13, "passing_yards":289, "rushing_yards":91, "fumbles":0 }));
...
let lostNames = lost.map(extractOpponent);
console.log(lostNames);
...
function gamesLost(lost) {
  console.log(lost.opponent +" at "+ lost.home +", "+ lost.opponent_score+" to "+ lost.home_score);
}
lost.forEach(gamesLost);
...
function fumble(array) {
  return (array.fumbles > 1 || array.fumbles == 1);
}

let gameFumble = huskyGames2016.filter(fumble);
console.log(gameFumble.length);
...
//Your function should handle the case where the first game has no
...
//then define a new function (called e.g., combinedFilter) that takes in a
...
//Note that fumbledAndLostFilter is a function!
...
let UWLost = huskyGames2016.filter(fumbledAndLostFilter);
console.log(UWLost);

//OPTIONAL extra practice: create a variable avgScoreDifference that
//represents the average number of points UW scored over their opponent
